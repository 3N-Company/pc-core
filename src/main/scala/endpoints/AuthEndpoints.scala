...
final class AuthEndpoints[F[_]: Monad: UserStorage: SessionStorage](baseEndpoints: BaseEndpoints[F]) extends EndpointsModule[F] {
  ...
  val login: ServerEndpoint[UsernamePassword, StatusCode, CookieValueWithMeta, Any, F] =
    endpoint
      .post
      .in("login")
      .in(auth.basic[UsernamePassword](WWWAuthenticate.basic(WWWAuthenticate.DefaultRealm)))
